...
namespace ShinyEggs.Repositories
{
	public class CartRepository : ICartRepository
	{
		...
		public async Task<int> GetCartItemCount(string userId = "")
		{
			...
			//return data.Count;
			//Console.WriteLine("HELPPPPPPPPPPPPPPPPPPPPPPP2");

			return data.Sum(a => a.Quantity);
		...
		public async Task<double> GetCartTotalAmount(string userId)
		{
			var cart = await GetCart(userId);
			if (cart == null)
			{
			...
			double cartTotalAmount = cart.CartDetails.Sum(item => item.Quantity * item.Products.Price);
