...
namespace ShinyEggs.Pages.Roles
{
	public class ManageModel : PageModel
	{
		...
		public ManageModel(ShinyEggs.Data.ApplicationDbContext context,
        ...
        public async Task<IActionResult> OnPostAsync(string selectedusername, string selectedrolename)
        {
            ...
            if (!User.IsInRole("Root Admin") && !User.IsInRole("Roles Manager"))
            {
                TempData["message"] = "You are not authorized to perform this action.";
                return RedirectToPage("Manage");
            }

            // Your existing code for role assignment
        ...
        public async Task<IActionResult> OnPostDeleteUserRoleAsync(string delusername, string delrolename)
        {
            ...
            // Check if the user is authorized to perform the action
            if (!User.IsInRole("Root Admin") && !User.IsInRole("Admin") && !User.IsInRole("Roles Manager"))
            {
                TempData["message"] = "You are not authorized to perform this action.";
                return RedirectToPage("Manage");
            }
            ...
            // Check if the user belongs to the role they are trying to remove
            if (await _userManager.IsInRoleAsync(user, delrolename))
            {
                if (delrolename == "Admin" || delrolename == "Root Admin")
                {
                    TempData["message"] = "You cannot remove users from this role.";
                }
                else if (delrolename == "Roles Manager" && !User.IsInRole("Root Admin"))
                {
                    TempData["message"] = "You cannot remove users from this role.";
                }
                else
            ...
            {
                TempData["message"] = "User is not in the specified role.";
            }
